...
def test_the_review_language_is_stored_on_the_order(store: SQLiteOrderStore) -> None:
    """A retry has no session behind it, so the language must survive the round trip."""
    created = create_order(store, "a@b.co.za", "R", ["a.pdf"], 0, language="zu")
    assert store.get(created.id).language == "zu"


def test_orders_default_to_english(store: SQLiteOrderStore) -> None:
    assert create_order(store, "a@b.co.za", "R", ["a.pdf"], 0).language == "en"


def test_the_email_follows_the_review_language() -> None:
    message = build_message("a@b.co.za", "# Review", ["Huurkontrak.pdf"], 9,
                            "Critical", "abc123", language="af")
    assert "Jou Legal-Eye oorsig" in message.subject
    assert "Kritiek" in message.subject
    assert "aangeheg" in message.body_text
    assert "Aanhegsel" in message.body_html
    assert 'html lang="af-ZA"' in message.body_html


def test_a_translated_review_says_english_is_authoritative() -> None:
    """The analysis is reasoned in English and translated, so this is not boilerplate."""
    from backend import languages

    for code in ("af", "zu", "xh"):
        language = languages.get(code)
        message = build_message("a@b.co.za", "# Review", ["a.pdf"], 5,
                                "Elevated", "abc123", language=code)
        # Asserted against the pack itself rather than a hand-copied fragment,
        # so editing a translation cannot silently drop the notice.
        assert language.strings["authoritative"] in message.body_text
        assert language.strings["unreviewed"] in message.body_text
        assert message.attachment_bytes.startswith(b"%PDF")


def test_english_carries_no_translation_notice() -> None:
    from backend import languages

    assert languages.notices(languages.get("en")) == []
    assert len(languages.notices(languages.get("zu"))) == 2


def test_an_unknown_language_falls_back_to_english() -> None:
    """A review in the wrong language is a disappointment; a crash loses it."""
    from backend import languages

    assert languages.get("kl").code == "en"
    assert languages.get(None).code == "en"
    assert languages.get("").code == "en"


def test_every_language_defines_every_string_and_band() -> None:
    """A missing key would raise mid-render, after the analysis had been paid for."""
    from backend import languages

    english = languages.LANGUAGES["en"]
    for code, language in languages.LANGUAGES.items():
        assert set(language.strings) >= set(english.strings), code
        assert set(language.bands) == set(english.bands), code
        for band, (label, note) in language.bands.items():
            assert label and note, f"{code}/{band}"


def test_afrikaans_ocr_is_available_to_the_extractor() -> None:
    from backend import languages

    assert "afr" in languages.ocr_languages()
    assert "eng" in languages.ocr_languages()
